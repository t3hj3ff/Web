...
class HomeController extends Controller
{
            ...
            foreach ($posts as $post) {
                ...
                if (is_array($icalList)) {
                    foreach ($icalList as $item) {
                        ...
                        try {
                            $ical = new ICal($url, [
                                'defaultTimeZone' => get_option('timezone', 'Europe/London'),
                            ]);
                            if ($ical->hasEvents()) {
    ...
    public function _getIcalUrl(Request $request, $home_id)
    {
        ...
        date_default_timezone_set(get_option('timezone', 'Europe/London'));
    ...
    public function getById1($home_id, $global = false)
    {
        $home_object1 = new Home();
        $post_item1 = $home_object1->getById1($home_id);
        if (!is_null($post_item1)) {
            $post_item1 = $this->setup_post_data($post_item1);
        }
        if ($global) {
            global $post;
            $post = $post_item1;
    ...
    public function getById2($home_id, $global = false)
    {
        $home_object2 = new Home();
        $post_item2 = $home_object2->getById2($home_id);
        if (!is_null($post_item2)) {
            $post_item2 = $this->setup_post_data($post_item2);
        }
        if ($global) {
            global $post;
            $post = $post_item2;
    ...
    private function _storeData($post)
    {
        ...
        foreach ($taxonomies as $key => $taxonomy) {
            ...
            $post->$name = $term_relation_object->get_the_terms($post->post_id, $taxonomy->taxonomy_id);
        }
    ...
    public function _getHomeSingle(Request $request, $home_id, $home_name = null)
    {
        ...
        // $homeroomsObject = $this->getById1($home_id, true);
        // $homefeaturesObject = $this->getById2($home_id, true);
        $result1 = new Home();
        $finalres = $result1->getFeatures($home_id);
        $finalroom = $result1->getRooms($home_id);
        $finalallroom = $result1->getRoomsinner($home_id);
        $finalresult = json_decode(json_encode($finalres), true);
        $finalrooms = json_decode(json_encode($finalroom), true);
        $finalroomsinner = json_decode(json_encode($finalallroom), true);
        ...
        if (is_null($homeObject) || !$homeObject || $homeObject->status != 'publish') {
            ...
            return view('frontend.home.default',['result' => $finalresult,'rooms' => $finalrooms,'homeId' => $home_id,'finalrooms' => $finalroomsinner]);
        }
    }
