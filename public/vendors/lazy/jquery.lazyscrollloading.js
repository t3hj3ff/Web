...
(function($) {
	...
	$.extend($.fn, {
		 ...
		 * @return Object
		 */
		"getLazyScrollLoadingViewport" : function(options) {
			var $container = this;
			...
			if (containerViewport == null) {
				...
				containerViewport = {
					...
					"isScrollToBottom" : function() {
						...
						return (this.getScrollTop() + this.getHeight(true) + options.offsetBottom >= this.getScrollHeight(true));
					},
		 ...
		 * @return jQuery
		 */
		"destroyLazyScrollLoading" : function(options) {
			/* yield event handler */
			return this.each(function() {
				var $container = $(this);
				/* reset event handler */
				$container.getLazyScrollLoadingViewport(options).getScrollBindTarget().unbind("scroll." + PLUGIN_NAMESPACE);
				/* clear cache */
		 ...
		 * @return boolean
		 */
		"isLazyScrollLoadingLazyItemVisible" : function($container, options) {
			var lazyItemViewport = this.getLazyScrollLoadingViewport(options);
			var containerViewport = $container.getLazyScrollLoadingViewport(options);
