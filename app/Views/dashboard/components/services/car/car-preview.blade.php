...
<div id="hh-dashboard-service-preview" class="card-box">
    ...
    <div class="content pb-3">
        ...
        @if(!empty($langs))
            ...
            @foreach($langs as $key => $item)
                <span class="address has-translation {{$key != 0 ? 'hidden' : ''}}" data-hh-bind-from="#location_address_{{$item}}" data-lang="{{$item}}" data-hh-bind-default="{{__('Address')}}">{{__('Address')}}</span>
            @endforeach
            </p>
        @else
        ...
        <div class="facilities d-flex align-items-center justify-content-between">
            ...
            <div class="item max-bedrooms">
                <div class="d-block text-center">
                    ...
                    <span class="text-center mt-1" data-hh-bind-from="#gear_shift" data-hh-bind-default="{{__('Auto')}}">{{__('Auto')}}</span>
                </div>
            </div>
        ...
        <div class="clearfix">
            ...
            <div class="prices">
                ...
                @if($currency['position'] == 'left'){{$currency['symbol']}}@endif<span class="price" data-hh-bind-from="#base_price"
                    ...
                    @if($currency['position'] == 'right'){{$currency['symbol']}}@endif<span class="unit">/{{get_car_unit()}}</span>
            </div>
        </div>
    </div>
</div>
