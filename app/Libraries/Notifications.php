...
class Notifications
{
    ...
    public function _addNotiWhenCreatedBooking($booking_id)
    {
        ...
        if($booking->service_type == 'car'){
        	$message = sprintf('at <strong>%s</strong> from %s to %s', $serviceObject->post_title, date(hh_date_format(true), $booking->start_time), date(hh_date_format(true), $booking->end_time));
        }else{
	        $message = sprintf('at <strong>%s</strong> from %s to %s', $serviceObject->post_title, date(hh_date_format(), $booking->start_date), date(hh_date_format(), $booking->end_date));
        }
