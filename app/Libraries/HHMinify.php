...
class HHMinify
{
    ...
    public function renderCSS($in_header = true, $type = '')
    {
        ...
        $prefix = $in_header ? 'header' : 'footer';
        $prefix .= '-' . $name;
        $subfix = $type;
        $this->cache_path = public_path('caching');
        $minified_css_file = $this->cache_path . '/' . $prefix . '-minified-' . $subfix . '.css';
        ...
        if (is_file($minified_css_file)) {
            echo '<link href="' . asset('caching/' . $prefix . '-minified-' . $subfix . '.css') . '" rel="stylesheet">' . "\r\n";
        } else {
    ...
    public function renderJS($in_header = true, $type = '')
    {
        ...
        $subfix = $type;
        $this->cache_path = public_path('caching');
        $minified_js_file = $this->cache_path . '/' . $prefix . '-minified-' . $subfix . '.js';
        ...
        if (is_file($minified_js_file)) {
            echo '<script src="' . asset('caching/' . $prefix . '-minified-' . $subfix . '.js') . '"></script>' . "\r\n";
        } else {
            if (!is_dir($this->cache_path)) {
                File::makeDirectory($this->cache_path, $mode = 0755, true, true);
            }
